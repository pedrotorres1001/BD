...
```
Entidades: Produtos, Encomenda, Fornecedor, Tipo de Fornecedor, Items e Condições de pagamento.

Atributos: 
	Produtos: código, nome, preço, taxa de IVA, quantidade;
	Encomenda: número, data, nº produtos (> 1), identificador do produto;
	Fornecedor: nome, NIF, endereço, nº fax, condições de pagamento, código interno;
	Tipo de Fornecedor: código e designação;
	Items: quantidade;
	Condições de pagamento: código e designação;

Relações da base de dados: Items -> Produtos, Encomenda -> Fornecedor, Encomenda -> Items, Forncedor -> Tipo de Fornecedor e Fornecedor -> Condições de Pagamento.
```
...
```
Em relação ao grau temos sempre relações binárias, visto que ambas as relações contém apenas duas entidades. Em relação à obrigatoriedade não pode haver uma produtos sem items, não pode haver uma encomenda sem um fornecedor, não pode haver tipo de fornecedor sem um fornecedor e também não pode haver condições de pagamento sem um fornecedor. Por fim, em relação à cardinalidade para a relação Items -> Produtos temos N:1, para a relação Encomenda -> Fornecedor temos N:1, para Encomenda -> Items temos 1:N, para Forncedor -> Tipo de Fornecedor temos N:1 e, por fim, para Fornecedor -> Condições de Pagamento temos N:1. 
```
...
#### *a)* Desenvolva o desenho conceptual da base de dados do Sistema de Gestão de Conferências com recurso a um diagrama entidade-relacionamento.
